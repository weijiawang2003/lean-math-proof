...
Builds on NS6's credit-aware sweep, adding two new safeguards:

1. **Pre-flight rank-coupling detector** — before invoking `eval`,
   compare the bag's deterministic skeleton-emit order for the
   mutated candidate vs. the baseline. If any protected skeleton
   (built from the protected_skeletons.json file) is dropped from
   the mutated bag or pushed back past its baseline skeleton-emit
   rank, reject the candidate without paying for a Lean roundtrip.

2. **Credit-aware `archive_seed`** — replaces NS5/NS6's wins-only
   selector with a `direct_wins*10 + assist*5 + advances*1 - 10*regr`
   score, *and* unconditionally protects any skeleton with non-zero
   assist credit.

CLI:

    python -m evolve.skeleton_evolve_ns7 \\
        ...
        --baseline-traces project/evolve/ns7_runs/baseline/medium/eval-XXX/traces.jsonl \\
        --baseline-traces project/evolve/ns7_runs/baseline/large/eval-YYY/traces.jsonl \\
        --protected project/evolve/ns7_runs/baseline/protected_skeletons.json \\
        --out-dir project/evolve/ns7_runs \\
        --max-cycles 40
"""
...
from evolve.rank_coupling import check_rank_coupling, summarize_violations
...
# Default queue — covers all NS7 operators plus enough scoped reorders
# and safe-prune variants to fill 25-40 cycles.
DEFAULT_QUEUE: list[tuple[str, dict]] = [
    ("baseline", {}),
    # Safe pruning at increasing aggression (credit-aware).
    ...
    # Credit-aware archive_seed — should now reach 37/38.
    ("archive_seed_credit", {"top_n": 12}),
    ...
    # Wins-only archive_seed for comparison (expected to plateau lower).
    ("archive_seed", {"top_n": 18}),
    ("archive_seed", {"top_n": 22}),
    # Scoped reorders per (origin, shape) — known to be no-regression.
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("project/evolve/ns7_runs"))
    ...
    ap.add_argument("--preflight-disable", action="store_true",
                    help="Disable the pre-flight rank-coupling check.")
    args = ap.parse_args()

    run_id = f"ns7-{time.strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:6]}"
    ...
    # Load protected set.
    protected_data = json.loads(args.protected.read_text(encoding="utf-8"))
    protected_entries = protected_data["entries"]
    log(f"protected: {protected_data.get('skeleton_count')} skeletons, "
        f"{len(protected_entries)} entries from {args.protected}")
    ...
    config_path.write_text(json.dumps({
        ...
        "protected_path": str(args.protected),
        "ckpt_dir": args.ckpt_dir,
        "max_cycles": args.max_cycles,
        "preflight_enabled": not args.preflight_disable,
        "git_head": _git_head(),
    ...
    for cycle_idx, (op_name, raw_kwargs) in enumerate(queue, start=1):
        ...
        # PRE-FLIGHT rank-coupling check.
        preflight_violations = 0
        preflight_summary: dict | None = None
        preflight_rejected = False
        if not args.preflight_disable and op_name != "baseline":
            violations = check_rank_coupling(
                best_genome, cand, protected_entries,
            )
            preflight_violations = len(violations)
            preflight_summary = summarize_violations(violations)
            if violations:
                preflight_rejected = True
                preflight_rejected_count += 1
                log(f"    PRE-FLIGHT REJECTED: {preflight_violations} violations "
                    f"(by_kind={preflight_summary['by_kind']}, "
                    f"affected_theorems={len(preflight_summary['affected_theorems'])})")
        ...
        if preflight_rejected:
            notes = (
                f"pre-flight rejected: {preflight_violations} violations "
                f"({preflight_summary['by_kind']})"
            )
        else:
            ...
            if proved_med_int >= baseline_floor:
                ...
                notes = (
                    f"rejected (Lean): proved={proved_med_int} < best_floor={baseline_floor}"
                )
            ...
            if promoted and n_large_evals < args.max_large_evals:
                ...
                if met_large:
                    ...
                    if best_proved_large is None or proved_large >= best_proved_large:
                        ...
                        log(f"    LARGE REGRESSION: {proved_large} < {best_proved_large}")
                        notes += f"; large regression {proved_large} < {best_proved_large}"
        ...
        with mutation_log.open("a", encoding="utf-8") as f:
            ...
            f.write(f"\n**preflight_rejected**: {preflight_rejected} (violations={preflight_violations})\n")
    ...
    log(f"DONE — best_cycle={best_cycle}, "
        f"proved_medium={best_proved_medium}, proved_large={best_proved_large}, "
