#!/usr/bin/env python3
"""RC4C Part 9 — determinism check.

Re-runs the gated d2 probe twice over known_wins_all + fresh_holdout_all +
negative controls and compares per-theorem (gate-fired, solved) outcomes. The gate is a
pure function, so gate decisions must be 100% stable; the solved outcomes are compared on
the cleanly-executed theorems only (heavy `<;> aesop` probes can hit the Dojo hard-timeout
and get worker-killed — those are infrastructure open-flakes, excluded from the hash and
reported separately, per the RC4B determinism lesson). Reports run1/run2 hashes, genuine
diffs, flake diffs, open flakes, deterministic true/false. `--analyze-only` recomputes the
verdict from saved run{1,2}.json without re-probing.
"""
...
SETS = ("known_wins_all", "known_wins_nonoverlap", "fresh_holdout_all",
...
def main():
    ...
    for setname in SETS:
        ...
        for e in json.load(open(_p(rel))):
            ...
            if fn in seen:
                continue
            seen.add(fn)
            fa, ma, tactics, na, la = G.gate_fires(
                policy, e.get("namespace"), e.get("goal_text"), fn, mode="all")
            targets.append({"full_name": fn, "file_path": e.get("file_path"),
                            "tactics": tactics if fa else None})

    nfire = sum(1 for t in targets if t["tactics"])
    cdir = _p("project/evolve/experiments/rc4_candidates/d2_simp_aesop/out/determinism_runs")
    os.makedirs(cdir, exist_ok=True)
    if args.analyze_only:
        run1 = json.load(open(os.path.join(cdir, "run1.json")))
        run2 = json.load(open(os.path.join(cdir, "run2.json")))
        print(f"[rc4c-determinism] analyze-only: run1={len(run1)} run2={len(run2)}", flush=True)
    else:
        print(f"[rc4c-determinism] run 1/2 over {len(targets)} targets ({nfire} gate-firing) ...",
              flush=True)
        run1 = _one_run(targets, args, os.path.join(cdir, "run1.json"))
        print("[rc4c-determinism] run 2/2 ...", flush=True)
    ...
    out = {"generated_by": "scripts/rc4c_determinism_check.py",
           ...
           "determinism_note": ("hash computed over cleanly-executed theorems; open flakes are "
                                "Dojo hard-timeout / worker-kill infrastructure events on heavy "
                                "`<;> aesop` goals, excluded from the hash."),
           "run1": run1, "run2": run2}
    json.dump(out, open(_p(args.out_json), "w"), ensure_ascii=False, indent=2)
    md = ["# RC4C determinism check", "",
    ...
    if flake_diffs:
        md += ["", "## Flake-induced diffs (excluded)", ""] + \
              [f"- `{d['full_name']}`: run1 solved={d['run1']['solved']} / run2 solved={d['run2']['solved']}"
               for d in flake_diffs]
    if flakes:
        md += ["", f"## Open flakes ({len(flakes)}) — infrastructure, excluded from hash", ""] + \
              [f"- {fn}" + ("  ⚠ credited-win" if fn in win_flakes else "") for fn in flakes]
    open(_p(args.out_md), "w").write("\n".join(md) + "\n")
    print(f"[rc4c-determinism] clean hash {h1} vs {h2} | genuine_diffs={len(genuine_diffs)} | "
