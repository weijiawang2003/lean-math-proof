#!/usr/bin/env python3
"""RC4B Part 5 — literal RC2 baseline over the validation sets.
...
def _reuse_map():
    ...
    for path in REUSE_SOURCES:
        ...
        for r in json.load(open(_p(path))).get("results", []):
            ...
            if fn in m:
                continue
            # TR6-style records carry rc2_finished directly
...
def driver(args):
    ...
    print(f"[rc4b-rc2] theorems={len(theorems)} reused={len(theorems)-len(live)-n_ckpt} "
    ...
    for ci, chunk in enumerate(chunks):
        ...
        print(f"[rc4b-rc2] chunk {ci+1}/{len(chunks)} ({len(chunk)}) ...", flush=True)
    ...
    out = {"generated_by": "scripts/rc4b_run_literal_rc2.py",
    ...
    print(f"[rc4b-rc2] done: {dict(hist)}; per-set={floors}")
...
def main():
    ...
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc4_candidates/disjoint_left_bridge/out/rc2_baseline")
    ap.add_argument("--checkpoint", default="project/evolve/experiments/rc4_candidates/disjoint_left_bridge/out/rc2_baseline_checkpoint.json")
