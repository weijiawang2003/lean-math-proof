#!/usr/bin/env python3
"""RC5V3 Part 4 — literal RC2 baseline over the large fresh eval batch.

All-fresh batch, runs live via rc4r_bench_common at the exact RC2 config. Chunked worker
subprocesses under an OS hard timeout. Checkpoint + resume (partial progress is valid).
"""
...
def main():
    ...
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc5_v3/out/rc2_bench")
    ap.add_argument("--checkpoint", default="project/evolve/experiments/rc5_v3/out/rc2_bench_checkpoint.json")
    ...
    manifest = _manifest_from_batch(args.batch, "project/evolve/experiments/rc5_v3/cases/_sets", "rc5v3_batch")
    ...
    out = {"generated_by": "scripts/rc5v3_run_rc2_baseline.py", "wrapper": args.rc2_wrapper,
           "num_theorems": len(recs), **roll, "results": recs}
    json.dump(out, open(_p(args.out_json), "w"), ensure_ascii=False, indent=2)
    md = ["# RC5V3 RC2 baseline", "", f"- theorems: {len(recs)} | {roll['status_histogram']}", "",
          "## By namespace", "", "| ns | n | solved |", "|---|---|---|"]
    for ns, d in roll["by_namespace"].items():
        md.append(f"| {ns} | {d['n']} | {d['solved']} |")
    open(_p(args.out_md), "w").write("\n".join(md) + "\n")
    print(f"[rc5v3-rc2] {roll['status_histogram']}")
