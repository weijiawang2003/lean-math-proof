#!/usr/bin/env python3
"""RC4A Part 5 — literal RC2 baseline over the validation sets.

Reuse-first: SF4 / TR2 / TR3 ran literal RC2 at the IDENTICAL config (rc2_release
wrapper, ns24, hybrid_evolved, top-k 8, max-steps 8), so those classifications are
reused verbatim; the rest run live via eval_rollout_all in chunked worker subprocesses
under an OS hard timeout. Records solved/failed/flake + trace paths per theorem and
emits the exact commands used.
"""
...
def driver(args):
    ...
    base = os.path.dirname(_p(args.manifest))
    theorems, seen = [], set()
    set_membership = {}
    ...
    for t in theorems:
        ...
        if fn in ckpt:
            ...
            live.append(t)
    print(f"[rc4a-rc2] theorems={len(theorems)} reused={len(theorems)-len(live)-sum(1 for t in theorems if t['full_name'] in ckpt)} "
          f"ckpt={sum(1 for t in theorems if t['full_name'] in ckpt)} live={len(live)}", flush=True)
    ...
    for ci, chunk in enumerate(chunks):
        ...
        print(f"[rc4a-rc2] chunk {ci+1}/{len(chunks)} ({len(chunk)}) ...", flush=True)
    ...
    out = {"generated_by": "scripts/rc4a_run_literal_rc2.py",
    ...
    open(_p(args.out_dir, "..", "literal_rc2_commands.sh") if False
         else _p(os.path.dirname(args.out_json), "literal_rc2_commands.sh"), "w").write("\n".join(sh) + "\n")

    print(f"[rc4a-rc2] done: {dict(hist)}; per-set={floors}")
...
def main():
    ...
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc4_candidates/def_unfold_simp/out/rc2_baseline")
    ap.add_argument("--checkpoint", default="project/evolve/experiments/rc4_candidates/def_unfold_simp/out/rc2_baseline_checkpoint.json")
