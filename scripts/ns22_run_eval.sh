#!/usr/bin/env bash
# NS22 — eval one configuration on one theorem set.
# Mirrors scripts/ns21_run_eval.sh.
#
# Usage:
#   bash scripts/ns22_run_eval.sh <mode> <variant> <set>
#
...
case "$MODE" in
  ...
  raw_routed)
    ...
    TAG="ns22_rawrouted_${VBASE}_${SET}"
    GENOME=""
    SINGLE_CKPT=""
    ;;
  wrap_routed)
    ...
    TAG="ns22_wraprouted_${VBASE}_${SET}"
    SINGLE_CKPT=""
    ;;
  *)
