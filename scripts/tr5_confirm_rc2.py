#!/usr/bin/env python3
"""TR5 Part 3 — confirm literal RC2 status (reuse-first, chunked live fallback).

TR3 / SF4 / TR2 all ran literal RC2 with the IDENTICAL config TR5 uses (rc2_release
wrapper, ns24 router, hybrid_evolved, top-k 8, max-steps 8), so their verified
classifications are REUSED on an exact theorem match. Any target with no identical-config
record is run live via eval_rollout_all in chunked worker subprocesses under an OS hard
timeout with a per-chunk checkpoint.

Classifications: CONFIRMED_RC2_FAILURE / RC2_SOLVED / OPEN_FLAKE / PATH_ERROR /
TRACE_INSUFFICIENT. Only CONFIRMED_RC2_FAILURE is eligible for TRUE_DELTA. Protected
...
# reuse sources, all run with the identical RC2 config
TR3_CONF = "project/evolve/experiments/tr3/out/tr3_rc2_confirmation.json"
...
def _reuse_map():
    m = {}
    for path, prov in ((TR3_CONF, "tr3_reused"), (SF4_CONF, "sf4_reused"), (TR2_CONF, "tr2_reused")):
        fp = _p(path)
        ...
        for r in d.get("results", []):
            ...
            if cls and fn not in m:
                m[fn] = {"classification": cls, "rc2_finished": r.get("rc2_finished"),
                         ...
                         "trace_path": r.get("trace_path"), "provenance": prov}
    return m
...
def driver(args):
    ...
    for c in pool:
        fn = c["full_name"]
        # treat the pool's known status as stale unless it has a verified reuse/ckpt record
        if fn in ckpt:
            results[fn] = ckpt[fn]
        elif fn in reuse:
            ...
            live_cases.append(c)
    print(f"[tr5-confirm] pool={len(pool)} reused={sum(1 for c in pool if c['full_name'] in reuse)} "
    ...
    for ci, chunk in enumerate(chunks):
        ...
        print(f"[tr5-confirm] chunk {ci+1}/{len(chunks)} ({len(chunk)} cases) ...", flush=True)
    ...
    for c in pool:
        ...
        recs.append({
            ...
            "target_category": c.get("target_category"), "cluster_id": c.get("cluster_id"),
            ...
            "provenance": r.get("provenance", "tr5_live"),
        })
    ...
    out = {
        "generated_by": "scripts/tr5_confirm_rc2.py",
    ...
    for r in recs:
        md.append(f"| `{r['full_name']}` | {r['target_category']} | {r['classification']} | "
                  f"{r['rc2_finished']} | {r['provenance']} |")
    open(_p(args.out_md), "w").write("\n".join(md) + "\n")
    print(f"[tr5-confirm] done: {dict(hist)} | confirmed={len(confirmed)} | prov={dict(prov)}")
...
def main():
    ...
    ap.add_argument("--out-dir", default="project/evolve/experiments/tr5/out/rc2_confirm")
    ap.add_argument("--checkpoint",
                    default="project/evolve/experiments/tr5/out/rc2_confirm_checkpoint.json")
