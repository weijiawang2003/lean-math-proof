#!/usr/bin/env python3
"""RC4B Part 10 — optional schema-native wrapper smoke.

Builds `rc4b_candidate_wrapper.json` as a functional copy of the frozen RC2 wrapper
with the four disjoint_left bridge tactics added in the narrowest schema-native way
possible — prepended to `priority_templates["any"]` (the SET_ITE_SIMP / RC2 precedent)
and gated by `theorem_name_tactic_gates` so each only emits on its namespace's disjoint
theorems. Then runs a small smoke over known_wins + the negative controls through the
real eval_rollout_all harness and compares to literal RC2:
  - known wins still solve through the wrapper
  - no obvious regressions on negative controls
...
WRAPPER_OUT = "project/evolve/experiments/rc4_candidates/disjoint_left_bridge/rc4b_candidate_wrapper.json"
SMOKE_SETS = ("known_wins", "disjoint_negative_controls", "namespace_negative_controls")
...
def build_wrapper():
    w = json.load(open(_p(RC2)))
    w = copy.deepcopy(w)
    pri = w.setdefault("priority_templates", {})
    anyl = list(pri.get("any", []))
    bridge = list(GATES.keys())
    # prepend bridge tactics not already present (mirrors SET_ITE_SIMP prepend)
    ...
    for t, subs in GATES.items():
        gates[t] = subs
    w["_rc4b_candidate_metadata"] = {
        "base": "RC2",
        "base_wrapper": RC2,
        "added_component": "disjoint_left bridge (RC4B candidate, off-by-default, NOT released)",
        "added_tactics_priority_any_prepended": bridge,
        "theorem_name_tactic_gates_added": GATES,
        "promotion_allowed": False,
        "rc1_wrapper_untouched": True,
        "rc2_wrapper_untouched": True,
        "ns24_router_untouched": True,
        "note": "Smoke-only schema-native copy. Name-substring gates approximate the "
                "external namespace+disjoint gate; the external additive evaluator remains "
                "the authority.",
    }
...
def driver(args):
    ...
    for ci, chunk in enumerate(chunks):
        ...
        print(f"[rc4b-smoke] chunk {ci+1}/{len(chunks)} ({len(chunk)}) ...", flush=True)
    ...
    known = [r for r in recs if "known_wins" in r["sets"]]
    ...
    summary = {
        "generated_by": "scripts/rc4b_schema_wrapper_smoke.py",
        "wrapper": wrapper, "num_smoke": len(recs),
        "known_wins_total": len(known), "known_wins_solved_by_wrapper": known_solved,
        "regressions": regressions, "new_wins": new_wins,
        "no_regression": not regressions,
        "note": "Name-substring gates approximate the external namespace+disjoint gate; "
                "wrapper search may differ slightly from single-shot probe. Smoke only.",
        "results": recs,
    }
    json.dump(summary, open(_p(args.out_json), "w"), ensure_ascii=False, indent=2)
    md = ["# RC4B schema-native wrapper smoke", "",
          f"- wrapper: `{wrapper}`",
    ...
    print(f"[rc4b-smoke] known_solved={known_solved}/{len(known)} regressions={regressions} "
          f"new_wins={new_wins}")
...
def main():
    ...
    ap.add_argument("--policy")  # accepted for CLI symmetry; wrapper is built from RC2
    ap.add_argument("--out-json")
    ap.add_argument("--out-md")
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc4_candidates/disjoint_left_bridge/out/schema_smoke")
