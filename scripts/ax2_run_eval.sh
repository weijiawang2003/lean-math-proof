#!/usr/bin/env bash
# AX2 — eval one configuration on one theorem set against the NS24 router.
#
# Usage:
#   bash scripts/ax2_run_eval.sh <tag> <genome|NONE> <set>
#
#   tag    : short label for the eval-run dir (raw / ns9 / ax1sym)
#   genome : path to a strategy-config JSON, or NONE for routed-raw (no wrapper)
#   set    : theorem-set name
#
# Configs used by AX2 Stage 3:
...
#
# The symbolic-action block is namespace-gated inside the wrapper, so
# non-List/Option sets are unaffected (verified in AX1).
set -euo pipefail
...
OUT="project/evolve/eval_runs/ax2_${TAG}_${SET}"
LOG="project/evolve/eval_runs/ax2_${TAG}_${SET}.log"
