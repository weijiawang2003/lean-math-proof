#!/usr/bin/env python3
"""RC5V3 Part 5 — RC4R static stage over the large fresh eval batch.

RC4R is purely additive (gated priority tactics): non-gate-firing theorems are forced RC4 ≡ RC2
(reuse the RC2 baseline); only gate-firing theorems run live. Reports RC4 new wins over RC2,
regressions (additive ⇒ 0), gate emissions, by namespace/component. Verifies whether the RC4R
"0 fresh delta" property persists at larger scale.
"""
...
import rc5v3_run_rc2_baseline as R  # reuse _manifest_from_batch
...
def main():
    ...
    ap.add_argument("--rc2-results", default="project/evolve/experiments/rc5_v3/out/rc5v3_rc2_baseline_results.json")
    ap.add_argument("--out-json")
    ap.add_argument("--out-md")
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc5_v3/out/rc4_bench")
    ap.add_argument("--checkpoint", default="project/evolve/experiments/rc5_v3/out/rc4_bench_checkpoint.json")
    ...
    manifest = R._manifest_from_batch(args.batch, "project/evolve/experiments/rc5_v3/cases/_sets", "rc5v3_batch")
    ...
    out = {"generated_by": "scripts/rc5v3_run_static_stage.py", "wrapper": args.rc4_wrapper,
    ...
    print(f"[rc5v3-static] {roll['status_histogram']} new_over_rc2={len(new_over_rc2)} {new_over_rc2} "
