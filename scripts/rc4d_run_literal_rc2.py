#!/usr/bin/env python3
"""RC4D Part 5 — literal RC2 baseline over the composition validation manifest.

Reuse-first, then live. RC4D's validation sets are assembled entirely from the RC4A/RC4B/RC4C
theorem sets, every one of which already had literal RC2 run at the IDENTICAL config
(rc2_release wrapper, ns24 router, hybrid_evolved, top-k 8, max-steps 8). So the literal RC2
classification for almost every theorem is reused verbatim from the components'
literal_rc2_results.json (exact theorem + exact config match) and the TR confirmations; only
genuinely-new theorems run live via eval_rollout_all in chunked worker subprocesses under an
OS hard timeout.
"""
...
# component literal RC2 results (exact config) take precedence, then the TR confirmations.
REUSE_RESULTS = [
    "project/evolve/experiments/rc4_candidates/def_unfold_simp/out/literal_rc2_results.json",
    "project/evolve/experiments/rc4_candidates/disjoint_left_bridge/out/literal_rc2_results.json",
    "project/evolve/experiments/rc4_candidates/d2_simp_aesop/out/literal_rc2_results.json",
]
REUSE_CONFIRMATIONS = [
...
def _reuse_map():
    ...
    for path in REUSE_CONFIRMATIONS:
        if not os.path.exists(_p(path)):
            continue
        for r in json.load(open(_p(path))).get("results", []):
            fn = r["full_name"]
...
def driver(args):
    ...
    print(f"[rc4d-rc2] theorems={len(theorems)} reused={len(theorems)-len(live)-n_ckpt} "
    ...
    for ci, chunk in enumerate(chunks):
        ...
        print(f"[rc4d-rc2] chunk {ci+1}/{len(chunks)} ({len(chunk)}) ...", flush=True)
    ...
    out = {"generated_by": "scripts/rc4d_run_literal_rc2.py",
    ...
    md = ["# RC4D literal RC2 baseline", "",
          f"- theorems: {len(recs)} | status: {dict(hist)}",
          f"- reused: {sum(1 for r in recs if (r['provenance'] or '').startswith('reused'))} | "
          f"live: {sum(1 for r in recs if r['provenance']=='rc4d_live')}", "",
          "## Per-set", "", "| set | n | solved | failed |", "|---|---|---|---|"]
    for s, f in floors.items():
        md.append(f"| {s} | {f['n']} | {f['solved']} | {f['failed']} |")
    open(_p(args.out_md), "w").write("\n".join(md) + "\n")

    sh = ["#!/bin/sh",
          "# Literal RC2 baseline (identical config to RC4A/RC4B/RC4C/TR3/TR5/TR6):",
    ...
    print(f"[rc4d-rc2] done: {dict(hist)}; per-set={floors}")
...
def main():
    ...
    ap.add_argument("--out-dir", default="project/evolve/experiments/rc4_candidates/composition_rc4d/out/rc2_baseline")
    ap.add_argument("--checkpoint", default="project/evolve/experiments/rc4_candidates/composition_rc4d/out/rc2_baseline_checkpoint.json")
